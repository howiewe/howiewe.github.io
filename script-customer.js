document.addEventListener('DOMContentLoaded', () => {
    ...
    const dbVersion = 1;
    function openDB() { return new Promise((resolve, reject) => { const request = indexedDB.open(dbName, dbVersion); request.onerror = event => reject(`無法開啟 IndexedDB 資料庫: ${event.target.errorCode}`); request.onsuccess = event => resolve(event.target.result); }); }
    ...
    function renderProducts() {
        ...
        const filteredProducts = allProducts.filter(p => { const matchesCategory = categoryIdsToDisplay === null || (p.categoryId && categoryIdsToDisplay.has(p.categoryId)); const matchesSearch = p.name.toLowerCase().includes(searchTerm); return matchesCategory && matchesSearch; });
        productList.innerHTML = '';
        if (filteredProducts.length === 0) { productList.innerHTML = '<p class="empty-message">此分類下無產品。</p>'; return; }
        filteredProducts.forEach(product => {
            const card = document.createElement('div'); card.className = 'product-card'; card.onclick = () => openDetailModal(product.id);
            const firstImage = (product.imageUrls && product.imageUrls.length > 0) ? product.imageUrls[0] : '';
            card.innerHTML = ` <div class="image-container"><img src="${firstImage}" class="product-image" alt="${product.name}" loading="lazy" style="width: ${product.imageSize || 100}%;"></div> <div class="product-info"><h3>${product.name}</h3><p class="price">$${product.price}</p></div> `;
    ...
    function openDetailModal(id) {
        ...
        if (product.imageUrls && product.imageUrls.length > 0) {
            detailMainImage.src = product.imageUrls[0]; detailMainImage.alt = product.name;
            product.imageUrls.forEach((url, index) => { const thumbItem = document.createElement('div'); thumbItem.className = 'thumbnail-item'; if (index === 0) thumbItem.classList.add('active'); thumbItem.innerHTML = `<img src="${url}" data-index="${index}" alt="產品縮圖 ${index + 1}">`; detailThumbnailList.appendChild(thumbItem); });
        } else { detailMainImage.src = ''; detailMainImage.alt = '無圖片'; }
        detailModal.classList.remove('hidden');
        setTimeout(() => { if (product.ean13) { const barcodeElement = document.getElementById('detail-barcode'); if (barcodeElement) { try { JsBarcode(barcodeElement, product.ean13, { format: "EAN13", displayValue: true, background: "#ffffff", lineColor: "#000000", height: 50, margin: 10 }); } catch (e) { console.error(`無法生成條碼: ${product.ean13}`, e); } } } }, 0);
    }
    
    detailThumbnailList.addEventListener('click', e => { if(e.target.tagName === 'IMG') { detailMainImage.src = e.target.src; document.querySelectorAll('#detail-thumbnail-list .thumbnail-item').forEach(item => item.classList.remove('active')); e.target.parentElement.classList.add('active'); } });
    ...
    function init() {
        ...
        async function loadData() {
            // **重要**: 對於部署在 GitHub Pages 的公開頁面，我們讓它直接讀取最新的 JSON 檔案
            // 這樣可以確保所有訪客看到的都是一樣的內容，且不需要他們瀏覽器的 IndexedDB
            try {
