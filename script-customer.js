// script-customer.js (最終修正版 - 動態修改 Viewport)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const originalViewportContent = viewportMeta ? viewportMeta.getAttribute('content') : 'width=device-width, initial-scale=1.0';
    ...
    function showSlide(index) { if (totalSlides <= 1) return; if (index >= totalSlides) index = 0; if (index < 0) index = totalSlides - 1; const sliderWidth = sliderWrapper.clientWidth; sliderWrapper.style.transform = `translateX(-${index * sliderWidth}px)`; currentSlideIndex = index; updateUI(); }
    ...
    function openDetailModal(id) {
        const product = allProducts.find(p => p.id === id); if (!product || !detailInfo || !sliderWrapper || !detailThumbnailList || !sliderDots) return; const category = allCategories.find(c => c.id === product.categoryId);
    ...
    function init() {
        ...
        if (categoryToggleBtn) { categoryToggleBtn.addEventListener('click', toggleSidebar); }
        if (sortBtn) { sortBtn.addEventListener('click', (e) => { e.stopPropagation(); sortOptionsContainer.classList.toggle('hidden'); }); }
        if (sortOptionsContainer) { sortOptionsContainer.addEventListener('click', (e) => { e.preventDefault(); const target = e.target.closest('a'); if (target) { currentSort.key = target.dataset.value; sortBtnText.textContent = target.textContent; sortOptionsContainer.classList.add('hidden'); renderProducts(); } }); }
        if (orderToggleBtn) { orderToggleBtn.addEventListener('click', () => { currentSort.order = (currentSort.order === 'asc') ? 'desc' : 'asc'; orderToggleBtn.dataset.order = currentSort.order; renderProducts(); }); }
